...
namespace DevCesio.DevForm.SQL.K3Cloud
{
    ...
    public class CommFunction : ICommFunction
    {
        ...
        private const string C_STKInStock_PUR_MRB = @"SELECT a.fskey,a.fsid FID,a.fsentry FENTRYID,a.Forgid,'采购入库-采购退料' 类型
	        ...
	        ,mtl.FNUMBER 物料编码,oe.FMUSTQTY 应收数量,oe.FREALQTY 实收数量,a.fssl 实退数量,unt.FNUMBER 单位,CONVERT(DECIMAL(18,4),oef.FPRICE) 单价,CONVERT(DECIMAL(18,4),oef.FTAXPRICE) 含税单价,CONVERT(DECIMAL(18,2),oef.FTAXRATE) 税率
	        ,ISNULL(stk.fnumber,' ') 仓库,ISNULL(FV.FNUMBER,'0') FV,ISNULL(F.FID,'0') FVV,ISNULL(F.FNUMBER,'0') 仓位,a.fspc 批号
	        ,ISNULL(us.FNUMBER,' ') 操作员,b.fscjqh 机器号,b.fsuser 用户,b.fspwd 密码
        FROM xbt_data a
        INNER JOIN  T_STK_INSTOCKENTRY oe ON a.Fsentry = oe.FENTRYID
        INNER JOIN  T_STK_INSTOCKENTRY_F oef ON oe.FENTRYID = oef.FENTRYID
        LEFT JOIN T_ORG_ORGANIZATIONS org ON a.Forgid = org.FORGID
        LEFT JOIN T_ORG_ORGANIZATIONS org2 ON a.fownerid = org2.FORGID
        LEFT JOIN T_BD_DEPARTMENT dep ON a.FsDepartid = dep.FDEPTID
        LEFT JOIN T_BD_SUPPLIER sup ON a.fsgysbh = sup.FSUPPLIERID
        ...
        private const string C_SALOUTSTOCK_SALRETURNSTOCK = @"SELECT A.fskey,A.fsid FID,A.fsentry FENTRYID,A.Forgid
            ...
            ,mtl.FNUMBER 物料编码,OE.FMUSTQTY 数量,A.fssl 实退数量,unt.FNUMBER 单位,CONVERT(DECIMAL(18,4),OEF.FPRICE) 单价,CONVERT(DECIMAL(18,4),OEF.FTAXPRICE) 含税单价,CONVERT(DECIMAL(18,2),OEF.FTAXRATE) 税率
	        ,ISNULL(stk.fnumber,' ') 仓库,ISNULL(FV.FNUMBER,'0') FV,ISNULL(F.FID,'0') FVV,ISNULL(F.FNUMBER,'0') 仓位,A.fspc 批号
            ,ISNULL(us.FNUMBER,' ') 操作员,B.fscjqh 机器号,B.fsuser 用户,B.fspwd 密码
        ...
        #region Modify
        private const string C_UpdateMo = @"UPDATE A SET FPICKMTRLSTATUS =
        CASE WHEN B.FPICKEDQTY = 0 THEN 1
        WHEN B.FMUSTQTY > B.FPICKEDQTY THEN 2
        WHEN B.FMUSTQTY = B.FPICKEDQTY THEN 3 ELSE 4 END
        ...
        GROUP BY A.FENTRYID)B ON A.FENTRYID = B.FENTRYID
        WHERE A.FENTRYID IN({0})
        UPDATE A SET FSTATUS = CASE WHEN FSTATUS = 5 THEN CASE WHEN B.FLAG = 0 THEN 6 ELSE FSTATUS END
        ELSE FSTATUS END
        ...
        private string UpdateMoEntry_Q_FPICKMTRLSTATUS(string pmoentryids)
        {
            ...
            try
            {
                ...
                //sql = string.Format(" UPDATE A SET FPICKMTRLSTATUS = CASE WHEN B.FPICKEDQTY = 0 THEN 1 WHEN B.FMUSTQTY > B.FPICKEDQTY THEN 2 WHEN B.FMUSTQTY = B.FPICKEDQTY THEN 3 ELSE 4 END FROM T_PRD_MOENTRY_Q A INNER JOIN(SELECT A.FENTRYID,SUM(PE.FMUSTQTY) FMUSTQTY,SUM(PQ.FPICKEDQTY) FPICKEDQTY FROM T_PRD_MOENTRY_Q A INNER JOIN T_PRD_PPBOMENTRY PE ON PE.FMOENTRYID = A.FENTRYID INNER JOIN T_PRD_PPBOMENTRY_Q PQ ON PE.FENTRYID = PQ.FENTRYID WHERE A.FENTRYID IN({0}) GROUP BY A.FENTRYID)B ON A.FENTRYID = B.FENTRYID WHERE A.FENTRYID IN({0}) ", pmoentryids);
                sql = string.Format(C_UpdateMo, pmoentryids);
                string str = SQLHelper.ExecuteNonQuery(sql);
